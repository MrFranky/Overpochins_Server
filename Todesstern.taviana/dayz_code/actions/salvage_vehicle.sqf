...
_RemovedPartsArray = ["motor","HitLF2Wheel","HitRF2Wheel","HitMissiles"/*,"HitBody","HitHull","HitVRotor","HitFuel","HitEngine"*/];
...

{
	...
	if !(_x in _RemovedPartsArray) then {
		...
		{if ((_type isKindOf _x) &&(_part == "PartWheel")) exitWith {_part = "PartWTruck3";};} count ["Kamaz_Base","MTVR","Ural_Base","Ural_Base_withTurret","V3S_Base"]; // LKWs geben nun LKW-Reifen
		...
		if (_damage < 1 /*&& _damage > 0*/) then { //Tempfix for issue where certain hitpoints on some vehicles do not get damaged and allow infinite removal 	// Unbeschädigte Teile können nun ausgeschlachtet werden.
